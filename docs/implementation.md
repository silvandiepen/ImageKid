...
The native products are developed as Swift packages and generated Xcode application targets in `apps/native-macos`, targeting macOS 14. Open `apps/native-macos/Package.swift` for package development, or generate `ImageKid.xcodeproj` from `project.yml` for signed application builds. Root helpers are available as `npm run native:build`, `npm run native:test`, and `npm run native:run`.

The package form keeps shared engines directly testable. The generated project supplies application targets, bundle metadata, sandbox entitlements, archive configuration, and release schemes. ImageKid Upscale, ImageKid Cutout, and ImageKid Slicer have signed Mac App Store archives and editable App Store Connect versions; the flagship products retain their separate release boundaries.
...
- Initial macOS companion targets exist for `ImageKid Upscale` and `ImageKid Cutout`. They share a focused batch queue, support drag/drop and Open panel input, process images one at a time on device through `ImageKidInference`, and write generated files to a sibling output folder or chosen destination. The upscaler supports Standard Core Image and Best Quality Core ML when a compatible local Real-ESRGAN package has been imported. Cutout supports automatic engine selection, Apple Vision, flat-backdrop removal, and Best Quality Core ML when a compatible local BiRefNet package has been imported, along with per-item preview/edit/retry/reveal actions and a remembered watched-folder workflow. The companion apps never download models or request network access. Imported packages are copied to the shared App Group model library when signed with `group.com.hakobs.imagekid`, with a development fallback to Application Support. A separate `cutout` SwiftPM command-line tool supports scripted batches and watched folders.
- An `ImageKid Slicer` macOS target implements the first release of the slicing companion: one source image, slice rectangles drawn and edited on the canvas, cutting guides that Auto Slice cuts along, an optional grid, built-in and user-saved templates, edge snapping to guides/grid/other slices/centre lines, per-slice locking, and an opt-in slices list for renaming. Save asks for a folder and writes one source-resolution file per slice, temporary-file-first, never overwriting an existing file and never touching the source. It is sandboxed with no network entitlement and no App Group, and shares only `ImageKidCore`'s geometry helpers. Its unit tests run under `swift test`; an XCUITest suite drives the app on the `ImageKidSlicer` scheme. A signed Mac App Store build is attached in App Store Connect; final production icon artwork, screenshots, and the human release pass remain. See [ImageKid Slicer](slicer.md).
...
- Sculptor runs end to end locally — import, rate, generate, inspect, export GLB — across `tools/sculptor-engine` (worker), `packages/ImageKidSculptorKit` (protocol, worker bridge, session, model download) and `apps/native-macos/Sources/ImageKidSculptor` (app). It installs its own model, falling back from the R2 mirror to upstream Hugging Face, and finds its own worker by walking up to a checkout, so a development build needs no configuration. TripoSR replaced SPAR3D as the V1 engine because SPAR3D's weights are gated; `engines/spar3d.py` exists but has never been executed. Verified on 19 assets spanning every Tiko Media category: animals and people reconstruct best, flags cannot work at all. The Sculptor target is **not sandboxed** and has **no app icon**: sandboxing needs the Python runtime packaged into the bundle (`scripts/bundle_runtime.sh` does it, at roughly +2.5 GB), and the icon set is empty pending artwork. Replacing the protocol's stage weights with measured timings also remains. See `docs/sculptor.md` "Phase 0 findings".
