...
/// The live Metal canvas renderer (iPad). Keeps a **committed** texture (the
/// document, rebuilt from its layers) and a **live** texture (the stroke in
/// progress, re-stamped whole each frame so within-stroke overlap never builds
/// up). Each frame draws the paper, then committed, then live — positioned by
/// the canvas transform (fit + zoom + pan).
///
...
/// through the shared `BrushCompositor`. Mirrors the macOS renderer.
