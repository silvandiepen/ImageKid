...
/// The brush editor controls — the content that exposes the engine's depth.
/// Every control binds to the model's working `brush`, and a live preview
/// re-renders through BrushKit's reference renderer on each change (cheap: one
/// short stroke). Save / Load handle `.inkbrush`. Hosted inside the shared
/// `FloatingToolPanel` by `InkaBrushPanel`.
struct BrushEditorControls: View {
    ...
    private var header: some View {
        HStack {
            ...
            Button { model.saveBrush() } label: { Image(systemName: "square.and.arrow.down") }
                .help("Save as .inkbrush")
            Button { model.loadBrush() } label: { Image(systemName: "square.and.arrow.up") }
                .help("Load an .inkbrush")
        }
    }
